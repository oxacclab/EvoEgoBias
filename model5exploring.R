...
runModel <- function(spec) {
  ...
  data <- evoSim(agentCount = spec$agents,
                 ...
                 # last change uncapping advice
                 getAdviceFun = 
                 #   function(modelParams, agents, world, ties) {
                 #   adviceNoise <- 0
                 #   mask <- which(agents$generation == world$generation)
...
for(rd in rawdata) {
  ...
  allAgents <- rbind(allAgents, rd$agents[rd$agents$generation%%10 == 1, ])
}
ggplot(allAgents[allAgents$startingEgoBias==.99, ], 
       aes(x=generation, y=egoBias)) +
  geom_hline(yintercept = 0.5, linetype = 'dashed') +
  stat_summary(geom = 'point', aes(group = startingEgoBias), fun.y = mean, size = 3, alpha = 0.25) +
  stat_summary(fun.data = mean_cl_boot, fun.args=(conf.int = .99), 
               geom = 'errorbar', aes(group = startingEgoBias), size = 1) +
