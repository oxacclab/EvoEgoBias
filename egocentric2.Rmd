...
for(g in 1:generationCount) {
  print(paste0('Generation ',g,'. Mean egoBias = ',round(mean(agents$egoBias),3)))
  # Make decisions
...
print(paste0('Elapsed time: ',tElapsed,'s'))
# stopCluster(cores)
load("model1.RData")
...
rm('model1Summary')
for(model in model1) {
  tmp <- lm(successes ~ egoBias, data = model$agents[which(as.numeric(model$agents$generation)<201),])
  tmpDF <- as.data.frame(model[1:6,9]) # model properties
  tmpDF <- cbind(tmpDF, data.frame(intercept = tmp$coefficients[1], egoBiasCoef = tmp$coefficients[2]))
